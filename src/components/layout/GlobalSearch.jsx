...
/**
 * ---------------------------------------------------------------------------
 * SERVER SKETCH (Node/Express + Postgres example) — adapt to your stack.
 * Point SEARCH_ENDPOINT above at this route once it's live.
 * ---------------------------------------------------------------------------
 *
 ...
 *
 *   // Use a trigram or full-text index (pg_trgm / tsvector) for speed at scale.
 *   // Scope results to what the logged-in user's role is allowed to see.
 *   const [patients, doctors, appointments, medicines, billing] = await Promise.all([
 *     db.query(
 *       `SELECT id, name AS title, ward || ' · ' || department AS subtitle
 *        FROM patients WHERE name ILIKE $1 LIMIT 5`, [`%${q}%`]
 *     ),
 *     db.query(
 *       `SELECT id, name AS title, department AS subtitle
 *        FROM doctors WHERE name ILIKE $1 LIMIT 5`, [`%${q}%`]
 *     ),
 *     db.query(
 *       `SELECT id, patient_name || ' — ' || doctor_name AS title, scheduled_at AS subtitle
 *        FROM appointments WHERE patient_name ILIKE $1 LIMIT 5`, [`%${q}%`]
 *     ),
 *     db.query(
 *       `SELECT id, name AS title, stock_note AS subtitle
 *        FROM medicines WHERE name ILIKE $1 LIMIT 5`, [`%${q}%`]
 *     ),
 *     db.query(
 *       `SELECT id, description AS title, status AS subtitle
 *        FROM bills WHERE description ILIKE $1 LIMIT 5`, [`%${q}%`]
 *     ),
 *   ]);
 *
 *   const results = [
 *     ...patients.rows.map(r => ({ ...r, category: "patients" })),
 *     ...doctors.rows.map(r => ({ ...r, category: "doctors" })),
 *     ...appointments.rows.map(r => ({ ...r, category: "appointments" })),
 *     ...medicines.rows.map(r => ({ ...r, category: "medicines" })),
 *     ...billing.rows.map(r => ({ ...r, category: "billing" })),
 *   ];
 *
 *   res.json({ results });
 * });
